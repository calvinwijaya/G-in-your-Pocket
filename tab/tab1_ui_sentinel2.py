...
from PyQt6.QtWidgets import (
    ...
    QLineEdit, QDateEdit, QSlider, QTextEdit, QHBoxLayout
)
from PyQt6.QtCore import Qt, QDate, pyqtSlot, pyqtSignal
from PyQt6.QtGui import QIcon
from tab.tab1_sentinel2 import authenticate_and_initialize, process_sentinel2
...
class Sentinel2Tab(QWidget):
    ...
    def init_ui(self):
        ...
        self.process_btn.clicked.connect(self.process_sentinel2_data)
    ...
    def process_sentinel2_data(self):
        if not self.geometry:
            self.log("Load GeoJSON first!")
            return
        self.s2_clipped = process_sentinel2(self.geometry, self.start_date_input.date().toString("yyyy-MM-dd"), 
                                            self.end_date_input.date().toString("yyyy-MM-dd"), self.max_cloud_prob)
        self.processed_signal.emit(self.s2_clipped)
        self.log("Sentinel-2 data processed successfully.")

    def generate_map(self):
        if not self.s2_clipped:
            self.log("Process Sentinel-2 data first!")
            return
        self.log("Generating map...")
        geojson_geometry = self.geometry.getInfo()
        generate_map(self.s2_clipped, geojson_geometry, "Sentinel-2")
    ...
    def reset_parameters(self):
        """Resets Sentinel-2 processing parameters without re-authenticating."""
        self.s2_clipped = None
        self.geometry = None
